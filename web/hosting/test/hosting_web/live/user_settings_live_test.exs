defmodule HostingWeb.UserSettingsLiveTest do
  use HostingWeb.ConnCase
  ...
  describe "deleting account" do
    ...
    test "redirects to home page", %{conn: conn, user: user} do
      {:ok, view, _html} =
        conn
        |> log_in_user(user)
        |> live(url(~p"/users/settings"))

      view
      |> element("#delete-account")
      |> render_click()
      |> follow_redirect(conn, url(~p"/"))
  ...
  describe "Settings page" do
    ...
    test "redirects if user is not logged in", %{conn: conn} do
      assert {:error, redirect} = live(conn, url(~p"/users/settings"))
  ...
  describe "update email form" do
    ...
    test "updates the user email", %{conn: conn, password: password, user: user} do
      ...
      {:ok, lv, _html} = live(conn, url(~p"/users/settings"))
  ...
  describe "confirm email" do
    ...
    test "updates the user email once", %{conn: conn, user: user, token: token, email: email} do
      {:error, redirect} = live(conn, url(~p"/users/settings/confirm_email/#{token}"))
    ...
    test "does not update email with invalid token", %{conn: conn, user: user} do
      {:error, redirect} = live(conn, url(~p"/users/settings/confirm_email/oops"))
      assert {:live_redirect, %{to: path, flash: flash}} = redirect
      assert path == ~p"/users/settings"
      assert %{"error" => message} = flash
      assert message == "Email change link is invalid or it has expired."
