defmodule Hosting.AccountsTest do
  use Hosting.DataCase, async: true

  alias Hosting.Accounts
  alias Hosting.Accounts.{User, UserToken}
  ...
  describe "delete_user/1" do
    test "deletes the user, its site and domains if it's not been shared" do
      ...
      assert_raise(Ecto.NoResultsError, fn ->
        Hosting.Repo.get_by!(Sites.Site, id: site.id)
      end)

      assert_raise(Ecto.NoResultsError, fn ->
        Hosting.Repo.get_by!(Domains.Domain, id: domain.id)
      end)
    end
    ...
    test "doesn't delete shared sites" do
      ...
      assert Hosting.Repo.get_by!(Sites.Site, id: site.id)
      assert Hosting.Repo.get_by!(Domains.Domain, id: domain.id)
    end
  end
  ...
  describe "register_user/1" do
    test "sets up default site" do
      user = user_fixture() |> Repo.preload(:sites)
      ...
      assert domain.name =~ ~r/site[a-z0-9]+\.code\.test/
    end
