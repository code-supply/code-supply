...
cuboid(size=[leg_width, base_depth, bracket_height], chamfer=chamfer, edges=FRONT) {
  align(BACK + TOP)
    down(bracket_height / 2)
      fwd(bracket_height)
        rotate([-backward_lean, 0, 0])
          cuboid(size=[leg_width, bracket_height, rise]) {
            align(FRONT + TOP)
              back(bracket_height)
                rotate([shelf_angle, 0, 0])
                  cuboid(size=[leg_width, shelf_depth, bracket_height]) {
                    align(FRONT + TOP)
                      back(bracket_height)
                        cuboid(size=[leg_width, bracket_height, lip_height], chamfer=chamfer, edges=TOP);
                  }
            ;
            ...
            align(FRONT + TOP)
              down(upper_brace_side)
                rotate([-(90 - brace_other_angle), 0, 0])
                  cuboid(size=[leg_width, upper_brace_hypotenuse, bracket_height]);
          }
  ;
      ...
      down(bracket_height)
        fwd(bracket_height / 2)
          rotate([45 - backward_lean, 0, 0])
            cuboid(size=[leg_width, lower_brace_length, bracket_height]);
}
